...
rvmeta.readDataByGene <- function(scoreTestFiles, covFiles, geneFile, gene) {
  ...
  if (is.null(covFiles)) {
    .Call("rvMetaReadDataByGene", scoreTestFiles, "", geneFile, gene, PACKAGE="seqminer");
  } else {
    .Call("rvMetaReadDataByGene", scoreTestFiles, covFiles, geneFile, gene, PACKAGE="seqminer");
  }
};
...
rvmeta.readScoreByRange <- function(scoreFile, tabixRange) {
  ...
  storage.mode(tabixRange) <- "character"
  .Call("readScoreByRange", scoreFile, tabixRange, PACKAGE="seqminer");
};

#' Read tabix file
#'
#' @param tabixFile score test output file (any gzipped, tabix indexed file)
#' @param tabixRange a text indicating which range in the VCF file to extract. e.g. 1:100-200
#' @return score test statistics in given range
